...
mod implementation {
    ...
    #[link(name = "kernel32")]
    extern "system" {
        fn CreatePseudoConsole(size: Coord, input: Handle, output: Handle, flags: u32, pseudoconsole: *mut Hpc) -> HResult;
    ...
    fn coord(grid: GridSize) -> io::Result<Coord> {
        let cols = i16::try_from(grid.cols).map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "columns exceed ConPTY COORD"))?;
        let rows = i16::try_from(grid.rows).map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "rows exceed ConPTY COORD"))?;
        Ok(Coord { x: cols, y: rows })
    }

    /// Owns only the HPCON handle. Pipe lifetime is owned by the caller's session worker.
    pub struct PseudoConsole { handle: Hpc }

    impl PseudoConsole {
        /// `input` is terminal-to-child; `output` is child-to-terminal. Both handles must use
        /// synchronous I/O per the ConPTY contract.
        pub unsafe fn create(input: RawHandle, output: RawHandle, grid: GridSize) -> io::Result<Self> {
            let mut handle = MaybeUninit::<Hpc>::uninit();
            let hr = CreatePseudoConsole(coord(grid)?, input as Handle, output as Handle, 0, handle.as_mut_ptr());
            if hr < 0 { return Err(io::Error::from_raw_os_error(hr)); }
            Ok(Self { handle: handle.assume_init() })
        }
...
#[cfg(windows)]
pub fn is_supported() -> bool { implementation::is_supported() }
