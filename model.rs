...
impl AgentState {
    pub fn needs_attention(&self) -> bool {
        matches!(self, Self::WaitingPermission | Self::WaitingUserInput | Self::Failed)
    }
...
impl AgentSnapshot {
    ...
    pub fn apply(&mut self, state: AgentState, source: EvidenceSource, summary: impl Into<String>) {
        ...
        let strongest = self.evidence.iter().map(|e| e.source.confidence()).max().unwrap_or(0);
        if source.confidence() >= strongest {
            self.state = state;
        }
        self.last_activity = now;
        self.evidence.push(AgentEvidence { source, observed_at: now, summary: summary.into() });
...
impl GridSize {
    pub fn new(cols: u16, rows: u16) -> Result<Self, &'static str> {
        if cols == 0 || rows == 0 { return Err("terminal grid must be non-zero"); }
        if cols > 1000 || rows > 1000 { return Err("terminal grid exceeds safe upper bound"); }
        Ok(Self { cols, rows })
    }
}
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        match self {
            ...
            Self::Split { ratio, first, second, .. } => {
                if !(0.1..=0.9).contains(ratio) { return Err("split ratio must be between 0.1 and 0.9"); }
                first.validate()?;
                second.validate()
            }
        }
...
impl SessionMetadata {
    pub fn new(id: impl Into<String>, profile: impl Into<String>, cwd: impl Into<String>, grid: GridSize) -> Self {
        Self { id: id.into(), profile: profile.into(), cwd: cwd.into(), grid, lifecycle: SessionLifecycle::Creating, history: HistoryPolicy::MetadataOnly }
    }
}
...
mod tests {
    ...
    fn layout_rejects_unsafely_small_split() {
        let layout = LayoutNode::Split {
            ...
            ratio: 0.05,
            first: Box::new(LayoutNode::Pane { pane_id: "a".into() }),
            second: Box::new(LayoutNode::Pane { pane_id: "b".into() }),
        };
    ...
    fn generic_session_remains_running_when_only_a_weak_pattern_arrives() {
        ...
        snapshot.apply(AgentState::WaitingPermission, EvidenceSource::OutputPattern, "possible approval text");
        assert_eq!(snapshot.state, AgentState::Running);
        assert_eq!(snapshot.evidence.last().unwrap().source, EvidenceSource::OutputPattern);
    }
