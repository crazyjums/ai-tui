...
function App() {
  ...
  const [activeId, setActiveId] = useState("claude-backend");
  ...
  function addSession(kind: "claude" | "codex" | "ssh") {
    const id = `${kind}-${Date.now()}`;
    const newSession: Session = {
      id,
      title: kind === "claude" ? "Claude — new task" : kind === "codex" ? "Codex — new task" : "SSH prod",
      kind,
      status: kind === "ssh" ? "idle" : "running",
      project: kind === "ssh" ? "Production" : "AI-CICD",
      cwd: kind === "ssh" ? "/srv/api" : "D:\\codes\\ai-cicd",
      duration: "00:00",
      detail: kind === "ssh" ? "Connected profile" : "Starting agent session",
  ...
  return (
    <div className="app-shell">
      ...
      <section className="tabbar">
        ...
        <button className="chrome-button" title="New terminal" onClick={() => addSession("claude")}><Plus size={19} /></button>
