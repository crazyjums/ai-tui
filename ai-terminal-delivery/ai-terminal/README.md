...
## Quick start: local terminal MVP

The browser UI now includes a real local PowerShell MVP. The Vite development server owns one persistent local shell session and exposes it only on loopback; the UI sends commands to that session and preserves its working directory between commands. Agent and SSH panes remain interaction prototypes until their adapters are implemented.
...
Open the URL printed by Vite and select the **PowerShell · Local** pane. Type commands in the input at the bottom and press **Run**. Try these interactions:
...
The local API intentionally runs only during `pnpm dev`; `pnpm preview` is for static UI preview and does not start a shell API.
...
The Windows Release package now includes a native modern shell window. Running `AI-Terminal.exe` opens a dark workspace UI with a sidebar, session tabs, terminal pane, Agent Context panel and command bar. The terminal pane is connected to a real PowerShell process; `ssh` and `wsl` can be launched from it.

```powershell
.\AI-Terminal.exe
```

The executable is built from `native-modern/main.cpp` with the Windows native API and is copied to the repository root as `AI-Terminal.exe` for convenience. `AI-Terminal-Console.exe` is retained as the older console-only fallback. The service still contains the isolated ConPTY FFI boundary for the future multi-pane renderer.
